...
import { i18nLogin } from "@/i18n/View.js";
...
const login = async ({ valid }) => {
  if (!valid) return;
  try {
    const login = await accountStore.login(form_data);
    toast.add({
...
<template>
  ...
  >
    <header class="form-header">
      ...
      <div class="content">
        <h1>{{ i18nLogin.formHeaderTitle[langStore.lang] }}</h1>
        <p>{{ i18nLogin.formHeaderDescription[langStore.lang] }}</p>
      </div>
    </header>
    <FormField v-slot="$field" name="email">
      <InputGroup>
        ...
        <FloatLabel variant="on">
          ...
          />
          <label>{{ i18nLogin.formFieldEmail[langStore.lang] }}</label>
        </FloatLabel>
      </InputGroup>
      <Message
        v-if="$field.invalid"
      ...
      <InputGroup>
        ...
        <FloatLabel variant="on">
          <Password
            ...
            :disabled="accountStore.google_auth_loading"
          />
          <label>{{ i18nLogin.formFieldPassword[langStore.lang] }}</label>
        </FloatLabel>
      </InputGroup>
      <Message
        v-if="$field.invalid"
    ...
    <Button
      type="submit"
      :label="i18nLogin.formButtonSubmit[langStore.lang]"
      :loading="accountStore.login_loading"
      fluid
    />
    <div class="form-flex center">
      <hr class="flex-fill" />
      <strong class="my-4 small muted">
        {{ i18nLogin.formButtonsSplit[langStore.lang] }}
      </strong>
      <hr class="flex-fill" />
    </div>
    <Button
      ...
      icon="pi pi-google"
      icon-class="me-3"
      :label="i18nLogin.formButtonGoogle[langStore.lang]"
    ...
    <Button
      ...
      severity="contrast"
      variant="text"
      :label="i18nLogin.formButtonRegister[langStore.lang]"
      fluid
